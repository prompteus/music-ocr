...
class ArchitectureConfig(BaseArchitectureConfig):
    kind: Literal["glm_ocr"] = "glm_ocr"
    # extra fields are passed to GlmOcrConfig
...
class Preprocessor:
    ...
    def __init__(self, config: PreprocessorConfig, tokenizer: music_ocr.tokenizer.SpaceTokenizer) -> None:
        ...
        self.img_start_token_id = tokenizer.get_token_id("<IMG_START>")
        self.img_end_token_id = tokenizer.get_token_id("<IMG_END>")
        self.image_processor: Glm4vImageProcessor = Glm4vImageProcessor(**config.image_processor)
    ...
    def preprocess_one(
        ...
        """Preprocessing function for individual examples."""
        ...
        prefix_ids = (
            [self.bos_token_id, self.img_start_token_id]
            + [self.img_token_id] * n_img_tokens
            + [self.img_end_token_id, self.sep_token_id]
        )
        if label is None:
    ...
    def process_batch(self, batch: list[SingleInput]) -> tensordict.TensorDict:
        """Converts a list of preprocessed examples to model inputs and collates them together."""
...
def _(architecture_config: ArchitectureConfig) -> transformers.PreTrainedModel:
    ...
    return transformers.GlmOcrForConditionalGeneration(transformers.GlmOcrConfig(**params))
